...
s32 NHTTPi_SocSSLConnect(nhttp::Self *self, void *param_2,
  ...
  if (request->should_use_client_cert == 1) {
    s32 res = nhttp::SSLSetBuiltinClientCert(request->ssl_ctx,
                                             request->client_cert_index);
    if (res != 0) {
      return -1005;
    }
  } else if (request->buffer1 != nullptr && request->buffer2 != nullptr) {
    s32 res = nhttp::SSLSetClientCert(request->ssl_ctx, request->buffer1,
                                      request->buffer1_len, request->buffer2,
                                      request->buffer2_len);
    if (res != 0) {
      return -1005;
    }
  ...
  s32 ret =
      nhttp::SSLSetRootCA(request->ssl_ctx, wiilinkca_pub, wiilinkca_pub_size);
  if (ret != 0)
    return -1004;
...
DEMAE_DEFINE_PATCH = {
    Patch::WriteFunctionCall(0x802433dc, NHTTPi_SocSSLConnect),
    Patch::WriteFunctionCall(0x80245d70, NHTTPi_SocSSLConnect)};
} // namespace demae::NHTTP